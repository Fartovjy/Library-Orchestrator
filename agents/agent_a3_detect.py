...
def _detect_loop(self, worker_idx: int) -> None:
    ...
    while True:
        ...
        try:
            ...
            self._move_to_error_dir(task, reason=f"A3 detect: {str(exc)[:120]}")
...
def _is_book_candidate(self, task: FileTask) -> tuple[bool, str]:
    ...
    if ext == ".fb2":
        return True, "book_ext:.fb2"

    if ext in TEXT_EXTENSIONS:
        snippet = extract_text_snippet(path, max_chars=1200).lower()
        if any(token in snippet for token in BOOK_CONTENT_TOKENS):
            return True, "text_book_signals"
        # Имя вида «Автор — Название» — сильный сигнал в архивах библиотек
        parsed = parse_filename(path.stem)
        if len(parsed.get("author", "")) >= 3 and len(parsed.get("title", "")) >= 3:
            return True, "text_book_filename"
        # Имя вида «Фамилия_Имя_Название_Книги» (транслитерация с портала)
        if filename_looks_like_book_stem(path.stem):
            return True, "text_book_filename"
        # Числовой ID-суффикс — признак формата MyLib/Flibusta (Author.NNNNN.txt)
        if LIBRARY_PORTAL_ID_RE.search(path.stem):
            return True, "text_library_portal_id"
        # Сопровождающий .fbd файл — запасная проверка для нестандартных имён
        if path.with_suffix(".fbd").exists():
            return True, "text_fbd_sidecar"
    ...
    # Файлы с неизвестным/числовым расширением, которые filetype не опознал:
    # делаем последний шанс через сигнатуру (FB2-XML, PDF, DOC и т.д.).
    if not kind_ext and not mime:
        magic = sniff_book_magic(path)
        if magic:
            return True, f"book_magic:{magic}"
