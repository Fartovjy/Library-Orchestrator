...
Для `.fb2.zip` читается внутренний `.fb2`, поэтому дальше в A6 такая книга отображается и анализируется как FB2, а не как обычный ZIP-архив.
...
По умолчанию исходники сохраняются. Удаление исходников считается опасным режимом и включается только явно; даже тогда файл удаляется только после проверки, что упакованная копия существует и ее содержимое совпадает по `XXH64`.
