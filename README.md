...
По умолчанию исходники сохраняются. Удаление исходников считается опасным режимом и включается только явно; даже тогда одиночный исходный файл удаляется только после проверки, что упакованная копия находится внутри `TARGET_DIR`, существует и ее содержимое совпадает по `XXH64`. Исходные архивы, внутри которых найдены книги, автоматически не удаляются: архив нельзя считать безопасно замененным одной проверкой отдельного ZIP-результата.
