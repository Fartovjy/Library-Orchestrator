...
В приложении реализована полная поддержка стандартных горячих клавиш в полях ввода (вкладка «Настройки») и в логе событий (вкладка «Главная») для **английской** и **русской** раскладок клавиатуры:
- **Копировать**: `Ctrl+C` / `Ctrl+С`
- **Вставить**: `Ctrl+V` / `Ctrl+М`
- **Вырезать**: `Ctrl+X` / `Ctrl+Ч`
- **Выделить всё**: `Ctrl+A` / `Ctrl+Ф`

### Панель воркеров каждого агента
Справа от карточки каждого агента на вкладке «Главная» находится индикатор активности воркеров (потоков). Его ширина увеличена до **12 пикселей**, чтобы курсор мыши легко попадал на сегменты воркеров для отображения всплывающей подсказки (tooltip) с именем файла, который сейчас находится в обработке.
...
- Из-за высокой частоты обращений к дисковой системе (особенно при распаковке архивов на шаге `A2` и расчете хэшей на шаге `A4`) рекомендуется создать виртуальный диск в оперативной памяти (RAM-диск, например с помощью ImDisk или аналогов) и указать путь к папке `temp` (`TEMP_BASE`) на него. Это существенно ускорит обработку и убережет физический накопитель от лишней нагрузки по записи.
