...
class LibraryGUIApp:
    def __init__(self, root: tk.Tk, dnd_available: bool) -> None:
        ...
        self.temp_base_var = tk.StringVar(
            value=self._setting_str("TEMP_BASE", str(lp.DEFAULT_TEMP_BASE))
        )
        ...
        self.isbn_provider_var = tk.StringVar(
            value=self._setting_choice(
                "ISBN_PROVIDER",
                lp.DEFAULT_ISBN_PROVIDER if lp.DEFAULT_ISBN_PROVIDER in {"auto", "openlibrary", "googlebooks"} else "auto",
    ...
    def _build_settings_tab(self) -> None:
        ...
        def add_pair(
            ...
            var_b: Optional[tk.Variable] = None,
            width: int = 10,
        ) -> None:
            nonlocal row
            lbl_a = tk.Label(
                content, text=self.tr(label_a), font=self.font_legend,
            ...
            ent_a = tk.Entry(content, textvariable=var_a, font=self.font_legend, width=width)
            ent_a.grid(row=row, column=1, sticky="ew", padx=(0, 16), pady=2)
            if label_b and var_b is not None:
                lbl_b = tk.Label(
                    content, text=self.tr(label_b), font=self.font_legend,
                    bg=self._c("root_bg"), fg=self._c("text_secondary"), anchor="w",
                )
                lbl_b.grid(row=row, column=2, sticky="w", padx=(0, 6), pady=2)
                self.settings_text_widgets.append((lbl_b, label_b))
                ent_b = tk.Entry(content, textvariable=var_b, font=self.font_legend, width=width)
                ent_b.grid(row=row, column=3, sticky="ew", pady=2)
            row += 1
        ...
        def add_path_entry(
            ...
            nonlocal row
            lbl = tk.Label(
                content, text=self.tr(label_key), font=self.font_legend,
                bg=self._c("root_bg"), fg=self._c("text_secondary"), anchor="w",
            )
            lbl.grid(row=row, column=0, sticky="w", padx=(0, 8), pady=2)
            self.settings_text_widgets.append((lbl, label_key))
            ent = tk.Entry(content, textvariable=var, font=self.font_legend, width=width)
            ent.grid(row=row, column=1, columnspan=2, sticky="ew", padx=(0, 8), pady=2)
            btn = tk.Button(
                content, text=self.tr("browse"), font=self.font_legend,
        ...
        for la, na, lb, nb in [
            ...
            ("settings_lm_workers",      "LM_WORKERS",              "settings_rename_workers",       "RENAME_WORKERS"),
            ("settings_pack_workers",    "PACK_WORKERS",            "settings_max_parallel_archives","MAX_PARALLEL_ARCHIVES"),
            ("settings_queue_size",      "QUEUE_SIZE",              "settings_target_hash_workers",  "TARGET_HASH_SCAN_WORKERS"),
        ]:
            add_pair(la, self.worker_vars[na], lb, self.worker_vars[nb])

        # ── А6 — провайдер (жанр и метаданные) ───────────────────────────
        section("settings_lm_a6")
        add_entry("settings_lm_url",     self.lm_url_var,     width=36)
        add_entry("settings_lm_model",   self.lm_model_var,   width=24)
        add_entry("settings_lm_api_key", self.lm_api_key_var, width=36)

        # ── А7 — провайдер (переименование) ──────────────────────────────
        section("settings_lm_a7")
        add_entry("settings_lm_url_rename",     self.lm_url_rename_var,     width=36)
        add_entry("settings_lm_model_rename",   self.lm_model_rename_var,   width=24)
        add_entry("settings_lm_api_key_rename", self.lm_api_key_rename_var, width=36)
        ...
        lbl_lang = tk.Label(
            content, text=self.tr("settings_output_language"), font=self.font_legend,
            bg=self._c("root_bg"), fg=self._c("text_secondary"), anchor="w",
        )
        ...
        combo = ttk.Combobox(
            content, textvariable=self.output_language_var,
            values=("auto", "ru", "en"), width=10, state="readonly", font=self.font_legend,
        )
        ...
        for la, na, lb, nb in [
            ("settings_gui_width",            "GUI_WINDOW_WIDTH",             "settings_gui_height",           "GUI_WINDOW_HEIGHT"),
            ("settings_gui_font_main",        "GUI_FONT_MAIN_SIZE",           "settings_gui_font_title",       "GUI_FONT_TITLE_SIZE"),
            ("settings_gui_font_small",       "GUI_FONT_SMALL_SIZE",          "settings_gui_font_stats",       "GUI_FONT_STATS_SIZE"),
            ("settings_gui_font_counter",     "GUI_FONT_COUNTER_LABEL_SIZE",  "settings_gui_font_legend",      "GUI_FONT_LEGEND_SIZE"),
            ("settings_gui_font_agent_title", "GUI_FONT_AGENT_TITLE_SIZE",    "settings_gui_font_agent_value", "GUI_FONT_AGENT_VALUE_SIZE"),
        ]:
        ...
        v_metric = self.lm_number_vars.get("GUI_FONT_AGENT_METRIC_LABEL_SIZE") or self.gui_font_vars["GUI_FONT_AGENT_METRIC_LABEL_SIZE"]
        add_pair("settings_gui_font_agent_metric", v_metric)

        # ── Сохранить ─────────────────────────────────────────────────────
        sep_final = tk.Frame(content, height=1, bg=self._c("text_muted"))
        sep_final.grid(row=row, column=0, columnspan=4, sticky="ew", pady=(12, 8))
        row += 1
        save_btn = tk.Button(
            content,
        ...
        save_btn.grid(row=row, column=0, columnspan=2, sticky="ew", pady=(0, 4))
        self.settings_text_widgets.append((save_btn, "settings_save"))
        hint_restart = tk.Label(
            content,
        ...
        hint_restart.grid(row=row, column=2, columnspan=2, sticky="ew", padx=(12, 0), pady=(0, 4))
    ...
    def _build_config(self) -> lp.Config:
        ...
        return lp.Config(
            ...
            lm_url_rename=lm_url_rename,
            lm_model_rename=lm_model_rename,
